# 1. Locate an instance of the target process (cmd.exe)
$targetProcess = Get-Process -Name "cmd" -ErrorAction SilentlyContinue | Select-Object -First 1
if (!$targetProcess) {
    Write-Host "Target process (cmd.exe) not found." -ForegroundColor Red
    exit
}
$targetPID = $targetProcess.Id
Write-Host "Using cmd.exe with PID $targetPID as target process."
